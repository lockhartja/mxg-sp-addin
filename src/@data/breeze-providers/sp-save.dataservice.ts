...
export class SpSaveDataService {
    ...
    constructor(private entitySaveData: SpSaveDataSvcInit) {}

    handleAddChangeSet(
        entity: SpBaseEntity,
        contentId: number
    ): IOdataaChangeSet {
        const { saveContext, customDataSvcUtils: utils } = this.entitySaveData;
        ...
        if (
            entity.entityType.autoGeneratedKeyType !== AutoGeneratedKeyType.None
        ) {
    ...
    handleUpdateChangeSet(
        ...
        const rawEntity = em.helper.unwrapInstance(entity);
    ...
    > {
        const { saveContext, saveBundle } = this.entitySaveData;

        const dataService = saveContext.entityManager.dataService;

        saveContext.tempKeys = [];

        saveContext.originalEntities = [];

        const saveResult: XtendedSaveResult = {
            entities: [],
            entitiesWithErrors: [],
            keyMappings: [],
            deletedKeys: [],
        ...
        for (const entity of saveBundle.entities) {
            const eState = entity.entityAspect.entityState;

            const contentId = idx + 1;

            saveContext.originalEntities[idx] = entity;
            ...
            switch (eState) {
                ...
                case EntityState.Modified:
                    changeSets.push(
                        this.handleUpdateChangeSet(entity, contentId)
                    );
                    break;

                case EntityState.Deleted:
                    changeSets.push(
                        this.handleDeleteChangeSet(entity, contentId)
                    );
                    break;
        ...
        if (changeSets.length === 1) {
            ...
            const knownUri = (saveContext
                .originalEntities[0] as SharepointEntity).__metadata.uri;
    ...
    private makeSaveResult(
        ...
        originalEnity: SharepointEntity,
        tmpKey?: EntityKey
    ): void {
        const { saveContext, jsonResultAdapter: jra } = this.entitySaveData;
        ...
        if (
            ...
            saveContext.saveResult.entitiesWithErrors.push(
                originalEnity as any
            );
            return;
        }

        if (originalEnity.entityAspect.entityState.isDeleted()) {
            saveContext.saveResult.deletedKeys.push({
        ...
        if (tmpKey) {
            ...
            if (
                tempEntityType.autoGeneratedKeyType ===
        ...
        saveContext.saveResult.entities.push(rawEntity);
    ...
    private processServerResponse = (
        ...
        const { saveContext } = this.entitySaveData;

        saveContext.saveResult.httpResponse = batchResponse[0]
            ? batchResponse[0]
         ...
         */
        if (!batchResponse[0]) {
            const originalEnity = saveContext.originalEntities[0];
            const tmpKey = saveContext.tempKeys[0];
        ...
        for (const response of batchResponse[0]) {
            const originalEnity = saveContext.originalEntities[idx];
            const tmpKey = saveContext.tempKeys[idx];
    ...
    saveOdata(
        ...
        const { saveBundle } = this.entitySaveData;

        return new Promise((resolve, reject) => {
            if (saveBundle.entities.length === 1) {
                OData.read(
                    request,
                    (unusedData, svrResponse) =>
                        resolve([undefined, svrResponse as any]),
                    (error) => reject(error),
                    OData.jsonHandler
                );
            } else {
    ...
    save(): Promise<SaveResult> {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                resolve(saveContext.saveResult);
            } catch (e) {
                utils.createError(e);
                reject(e);
            }
