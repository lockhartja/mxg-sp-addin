...
export class SpDataService extends AbstractDataServiceAdapter {
    ...
    executeQuery(mappingContext: XtendedMappingCtx): Promise<any> {
    ...
    saveChanges(
        ...
        const saver = new SpSaveDataService({
            saveContext,
            saveBundle,
            jsonResultAdapter: this.jsonResultsAdapter,
            customDataSvcUtils: this.utils,
        });
        return saver.save();
    }
    ...
    visitNode = (
        ...
        if (metadata != null) {
            ...
            const et =
                entityTypeName &&
                mappingContext.entityManager.metadataStore.getAsEntityType(
                    entityTypeName,
                    true
                );
